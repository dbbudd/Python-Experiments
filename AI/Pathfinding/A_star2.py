...
class node:
    ...
    priority = 0 #priority = dist + remaining distance estimation
    ...
    def pathFind(the_map, n, m, dirs, dx, dy, xA, yA, xB, yB):
        ...
        open_notes_map = [] # map of open (not-yet-tried) nodes
        dir_map = [] # map of dirs
        row = [0] * n
        ...
        pq = [[], []] #priority quest of open (not-yet-tried) nodes
        pqi = 0 # priority queue index
        
        #create the start node and push inot list of open nodes
        n0 = node(xA, yA, 0, 0)
        ...
        # A* search
        while len(pq[pqi]) > 0:
            # get the current node w/ the highest prioruty
            ...
            #generate moves (child nodes) in all possible directions (dirs)
            for i in range(dirs):
                xdx = x + dx[i]
                ydy = y + dy[i]
                if not (xdx < 0 or xdx > n-1 or ydy < 0 or ydy > m - 1 or the_map[ydy][xdx] == 1 or closed_nodes_map[ydy][xdx] == 1):
                    ...
                    if open_nodes_map[ydy][xdx] == 0:
                        ...
                        # except the node to be replced will be ignored
                        # and the new node will be pushed in instead
                        while not (pq[pqi][0].xPos == xdx and pq[pqi][0].yPos == ydy):
                            heappush(pq[1 - pqi], pq[pqi][0])
                            heappop(pq[pqi])
                        heappop(pq[pqi]) # remove the target mode
