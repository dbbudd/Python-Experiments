...
#### IMPORT DATASET AND RUN TEST ###

csv = np.genfromtxt('dataset.csv', delimiter=",", dtype=None, names=True)
...
for row in csv:
    ...
    outputs = [float(output1), float(output2), float(output3)]
    inputs = [float(input1), float(input2), float(input3)]
    temp_x.append(outputs)
    temp_y.append(inputs)
