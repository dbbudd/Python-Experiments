...
# Dynamixels move as follows:
        Counter-clockwise when goto speed between 0-1023
        Clockwise when goto speed between 1024-2047
...
def jointMode(ID):
        print("Setting dxl",ID,"to joint mode.")
        sc.flush()
...
def wheelMode(ID):
        print("Setting dxl",ID,"to wheel mode.")
...
def moveJoint(ID, position, speed):
        print("Moving joint dxl",ID,"to position",position,"at speed", speed)
        sc.flush()
...
def moveWheel(ID, speed):
        ...
        if speed < 0:
                ...
                print("Moving wheel dxl",ID,"CW at speed", speed)
                speed = 1023 + speed
                ...
                if speed > 1023:
                        ...
                        speed = 1023
                print("Moving wheel dxl",ID,"CCW at speed", speed)
...
def moveForward(left_wheels, right_wheels, speed):

        print("Moving forwards at speed",speed)
        
        for wheel in left_wheels:
                moveWheel(wheel, speed)

        for wheel in right_wheels:
                moveWheel(wheel, -speed)


def moveBackward(left_wheels, right_wheels, speed):

        print("Moving backwards at speed", speed)
...
def stopWheels(wheels):
        
        print("Stopping all wheels")
        for wheel in wheels:
                moveWheel(wheel, 0)

def wheelTest(wheels):

        for wheel in wheels:
                print("Moving Dynamixel", wheel, "CCW at speed 200")
                moveWheel(wheel,200)
        ...
        for wheel in wheels:
                print("Moving Dynamixel", wheel, "CW at speed 200")
                moveWheel(wheel,-200)
...
try:
        ...
        moveForward(left_wheels, right_wheels, 500)
        ...
        moveBackward(left_wheels, right_wheels, 500)
        ...
        print("Attempt a sync write...")
        sync_write(wheels, 0x83, [200,200,200,200])
