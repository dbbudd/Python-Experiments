...
class robot(object):
	...
	def wheelMove(self, stateID="stop"):
		stateDict = wheelStats[stateID]
		for wheel in state.keys():
			getWheel(wheel).spinAtSpeed(state[wheel])
...
robotObj = ({"shoulder":shoulderObj, "pan":panObj, "tilt":tiltObj}, {"fl":flWheel, "fr":frWheel, "bl":blWheel, "br":brWheel})
...
def keypress(event):
        ...
        if x == "q":
                ...
                robot.wheelMove("left")
        elif x == "d":
                robot.wheelMove("right")
        elif x == "s":
                robot.wheelMove("back")
        elif x == "1":
                robot.getServo("pan").increaseAngle()
        elif x == "2":
                robot.getServo("pan").moveToAngle(0)
        elif x == "3":
                robot.getServo("pan").moveToAngle(45)
        elif x == "z":
                robot.getServo("tilt").moveToAngle(-90)
        elif x == "x":
                robot.getServo("tilt").moveToAngle(0)
        elif x == "c":
                robot.getServo("tilt").moveToAngle(90)
        elif x == "i":
                robot.getServo("shoulder").moveToAngle(0)
        elif x == "k":
                robot.getServo("shoulder").moveToAngle(90)
        else:
                print(event.char)
